...
After a SteamOS update, if the AppID input box stops working, refresh the plugin
from the **ValveOFF release bundle** (no source on this repo):

```bash
curl -fsSL https://raw.githubusercontent.com/dvahana2424-web/hammerdeckydowngrade/main/update-hammer-decky.sh | bash
```

Or re-run the full one-paste installer (also refreshes Hammer binaries from `bin/`).
...
- **hammersteam.so** — Hammer **1.1.9** LD_AUDIT library (`deck-exit-fix`; Wudrm/OST manifest providers safe for Game Mode; proactive Steam exit shutdown)
...
packages from [hammer-downgrader](https://github.com/dvahana2424-web/hamdeck/tree/hammer-1.1.7/Hammer%20cloud%20save/hamdeck-hammer-1.1.3/tools/hammer-downgrader).
