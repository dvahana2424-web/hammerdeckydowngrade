<#
    ...
    Downloads the ValveOFF payload, installs it to
    "C:\Program Files (x86)\ValveOFF", creates a Desktop shortcut and
...
$InstallDir = "C:\Program Files (x86)\ValveOFF"
$AppName    = 'ValveOFF'
$Version    = '1.0'
$Publisher  = 'ValveOFF'
$Parts      = @('ValveOFF.zip.001', 'ValveOFF.zip.002')
$ExtraDirs  = @('cache', 'specialpatch_cache')   # created empty after extract
...
# ---- Workspace ----------------------------------------------------------
$work = Join-Path $env:TEMP ("valveoff_" + [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $work -Force | Out-Null
$zipPath = Join-Path $work 'ValveOFF.zip'
...
function Get-File($url, $dest, $label) {
    for ($try = 1; $try -le 3; $try++) {
        ...
        try {
            ...
            $req.UserAgent        = 'ValveOFFInstaller'
            $req.Timeout          = 30000
...
try {
    ...
    # ---- Stop running ValveOFF ------------------------------------------
    Get-Process -Name 'ValveOFF','Uninstall' -ErrorAction SilentlyContinue |
    ...
    $exePath = Join-Path $InstallDir 'ValveOFF.exe'
    $icoPath = Join-Path $InstallDir 'valveoff.ico'
    ...
    $regKey = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\ValveOFF'
